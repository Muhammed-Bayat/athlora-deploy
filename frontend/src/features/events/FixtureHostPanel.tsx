...
export function FixtureHostPanel({ event, canOperate, isCoach }: { event: AthleticsEvent; canOperate: boolean; isCoach: boolean }) {
  ...
  useEffect(() => {
    let current = true;
    const load = () => void Promise.all([listFixtureInvitations(event.id), listFixtureRosters(event.id), listHostedFixtureResults(event.id)])
      .then(([inviteResponse, rosterResponse, resultResponse]) => {
        if (!current) return;
        setInvitations(inviteResponse.data);
        setRosters(rosterResponse.data);
  ...
  return <Card>
    ...
    {rosters.length > 0 && <section aria-labelledby={`fixture-rosters-${event.id}`}><h3 id={`fixture-rosters-${event.id}`}>Accepted</h3>{rosters.map(({ team, participants }, index) => <article key={team.workspaceId}><h4><ClubBadge name={team.workspaceName} branding={team.branding} size="sm" decorative />{team.workspaceName}</h4><p>{participants.length === 0 ? 'No athletes selected.' : participants.map((participant) => participant.athlete.name).join(', ')}</p>{isCoach && event.status !== 'scheduled' && index > 0 && team.status !== 'withdrawn' && <Button variant="ghost" onClick={() => void withdraw(team.workspaceId)} disabled={busy}>Record withdrawal</Button>}</article>)}</section>}
