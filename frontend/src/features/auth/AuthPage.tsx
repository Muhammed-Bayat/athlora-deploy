      ...
      {isCoach && <Card className={styles.members}>
        ...
        <form className={styles.inviteForm} onSubmit={(event) => void inviteMember(event)}>
          ...
          <div className={styles.inviteField}>
            ...
            <Select className={styles.roleSelect} id="workspace-invite-role" value={inviteRole} onChange={(event) => setInviteRole(event.target.value as 'coach' | 'assistant')} disabled={inviteBusy} options={[{ value: 'assistant', label: 'Assistant' }, { value: 'coach', label: 'Coach' }]} />
          </div>
          <div className={styles.inviteAction}><Button type="submit" disabled={inviteBusy}>{inviteBusy ? 'Sending...' : 'Create invitation'}</Button></div>
        </form>
