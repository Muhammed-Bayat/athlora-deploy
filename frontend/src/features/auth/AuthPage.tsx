      ...
      {isCoach && <Card className={styles.members}>
        ...
        <form className={styles.inviteForm} onSubmit={(event) => void inviteMember(event)}>
          ...
          <div className={styles.inviteField}>
            ...
            <select className={styles.roleSelect} id="workspace-invite-role" value={inviteRole} onChange={(event) => setInviteRole(event.target.value as 'coach' | 'assistant')} disabled={inviteBusy}><option value="assistant">Assistant</option><option value="coach">Coach</option></select>
          </div>
