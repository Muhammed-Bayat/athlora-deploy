...
export function LandingPage({ onLogin, onSignup, onPasswordHelp }: LandingPageProps) {
  ...
  return <div className={styles.page} ref={pageRef}>
    ...
    <header className={styles.header}><nav className={styles.nav} aria-label="Landing page"><a href="#cinematic-intro" className={styles.brandLink} aria-label="Athlora home"><Brand /></a><div className={styles.desktopActions}><AccountButton onClick={onLogin}>Log in</AccountButton><a className={`${styles.button} ${styles.secondary}`} href="/schedule">Schedule</a><a className={`${styles.button} ${styles.secondary}`} href="/stats">Stats</a><AccountButton primary onClick={onSignup}>Get started</AccountButton></div><button ref={menuButtonRef} className={styles.menuButton} type="button" aria-label="Open menu" aria-expanded={menuOpen} aria-controls="landing-mobile-menu" onClick={() => setMenuOpen(true)}><span /></button></nav></header>
    {menuOpen && <div id="landing-mobile-menu" className={styles.mobileMenu} role="dialog" aria-modal="true" aria-label="Navigation menu"><div><Brand compact /><button ref={closeButtonRef} type="button" aria-label="Close menu" onClick={() => setMenuOpen(false)}>+</button></div><nav aria-label="Mobile landing page">{chapters.slice(1).map(([id, label]) => <a href={`#${id}`} key={id} onClick={() => setMenuOpen(false)}>{label}</a>)}</nav><AccountButton onClick={onLogin}>Log in</AccountButton><a className={`${styles.button} ${styles.secondary}`} href="/schedule">Schedule</a><a className={`${styles.button} ${styles.secondary}`} href="/stats">Stats</a><AccountButton primary onClick={onSignup}>Get started</AccountButton><button type="button" className={styles.passwordButton} onClick={onPasswordHelp}>Forgot password</button></div>}
