...
export const getDisciplineStatistics: RequestHandler = async (req, res, next) => {
  try {
    ...
    res.json({ data: await disciplineAthleteStatistics(getPool(), workspaceId, req.params.id ? String(req.params.id) : null, season.selected === 'all' ? new Date().getUTCFullYear() : Number(season.selected)) });
