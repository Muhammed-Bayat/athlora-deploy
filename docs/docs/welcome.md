...
- **Landing page** — the hero renders the approved SDP-Landing track exactly (a pointer-following 2D SVG oval with 8 gradient lanes, start line, distance marks, lane numbers and a javelin throw sector) in a soft-feather circular window that uses the mockup's static alpha mask (no lens ring), with a cursor glow; the track is drawn with a stronger perspective tilt for extra depth. The scroll-driven lap keeps the lazy-loaded Three.js scene but upgrades its blocky marker to a stylized humanoid sprinter whose overlapping primitives make every joint connect, painted in the logo's crisp cyan/blue palette (vibrant cyan `#5CCCFE` torso and shins, deeper blue `#5AC9FE` forearms and thighs, navy `#001D3C` head and feet — no clothes, no skin). The lap brightens the 3D lane lines, enlarges the oval, anchors the run to the start line so one full lap completes exactly when the page is scrolled to the bottom, and the two bottom bands (testimonial and call-to-action) are slightly translucent so the runner and track stay visible there. Scroll easing smooths both the runner's progress and its turning (heading is eased through the shortest arc and the camera uses a capped, frame-rate-independent lerp), so even a very fast scroll stays smooth and crisp instead of snapping. The cyan accents are brightened (landing cyan token lifted to `#b4f8fe`, brighter 3D lane glow) while the dark aurora background gradient is preserved.
...
- **Track-side live logger** — the mobile-first 100m logger starts or resumes fresh in-progress events, records decimal finishes and incidents through serialized controls, and keeps the core athlete/timeline console usable if standings history is unavailable. Entry-type-valid correction and accessible undo recover from exact stale-version conflicts, event closure immediately exits active controls, completed events cannot reopen from stale local state, and keyboard focus returns predictably after modal mutations.
...
- **Quality gate** — lint, typecheck, Vitest/RTL, Supertest, the Playwright E2E suite (smoke + desktop/mobile vertical slice + axe audit) and the Docusaurus build are green. CI workflow is committed in `.gitea/workflows/ci.yml` and executed by a registered Gitea Actions runner on the university instance; the `e2e` job boots a Postgres service container and skips with a clear message until the Auth0/E2E credentials are configured as repository secrets.
