...
- **Landing page** — the hero now renders the approved SDP-Landing track exactly (a pointer-following 2D SVG oval with 8 gradient lanes, start line, distance marks, lane numbers and a javelin throw sector), replacing the previous WebGL hero. The scroll-driven lap keeps the lazy-loaded Three.js scene but upgrades its blocky marker to a stylized articulated humanoid sprinter with a running stride, and brightens the 3D track's lane lines (plus a higher lap opacity) so the lanes read clearly.
