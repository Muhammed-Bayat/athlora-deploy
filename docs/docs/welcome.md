...
- **Quality gate** — lint, typecheck, Vitest/RTL, Supertest, production builds, an informational Vitest V8 coverage report, the Playwright E2E suite (smoke + desktop/mobile vertical slice + axe audit), and the Docusaurus build are configured in CI. The Gitea `coverage` job prints a detailed Markdown summary and uploads HTML/LCOV artifacts; the `e2e` job boots a Postgres service container and skips with a clear message until the Auth0/E2E credentials are configured as repository secrets.
