...
- **Landing page** — the hero renders the approved SDP-Landing track exactly (a pointer-following 2D SVG oval with 8 gradient lanes, start line, distance marks, lane numbers and a javelin throw sector) in a soft-feather circular window that uses the mockup's static alpha mask (no lens ring), with a cursor glow; the track is drawn with a stronger perspective tilt for extra depth. The scroll-driven lap keeps the lazy-loaded Three.js scene but upgrades its blocky marker to a stylized articulated humanoid sprinter dressed in the logo palette (cyan singlet, mint shorts, navy head), brightens the 3D lane lines, enlarges the oval, and anchors the run to the start line so one full lap completes exactly when the page is scrolled to the bottom.
