...
- Multi-discipline meet detail: `MeetRosterPanel` manages catalogue sessions, athlete/guest/relay entrants with ordered legs, session registration, and coach-only relay roster edits while scheduled. `SessionLivePanel` provides session-scoped timed logging with offline enqueue, attempt history, coach official-entry selection, read-time standings, and CSV export; it renders only for `discipline: null` meets. Public Stats club mode includes published session/team standings via `PublicSessionResults` with safe member summaries.
