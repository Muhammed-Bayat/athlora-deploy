...
- Tests: Vitest + React Testing Library cover the app shell, shared Button, API response/error handling, authenticated synchronization, athlete/event/participant/timeline/result/aggregate wrappers, roster workflows, event management, live logging, result ordering/ties, training, every incident, partial/history states, PB/SB, and set/clear/failure correction paths. Runs with `npm run test` (132 tests).
