...
- The app shell renders with **Athlora** branding and an ink sidebar. The roster is live against the typed API; dashboard and event content still use staged preview data until their integration issues land.
...
- Tests: Vitest + React Testing Library cover the app shell, shared Button, API response/error handling, authenticated synchronization, athlete wrappers, and roster loading, retry, empty/filter, create, edit, validation, archive, restore and keyboard workflows. Runs with `npm run test` (31 tests).
