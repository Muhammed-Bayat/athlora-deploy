...
- Tests: Vitest + Supertest (app routes, athlete, event and participant route suites), middleware/application-user resolution, ownership/non-disclosure checks, athlete/event/participant services, validation and row-mapping, result derivation, and migration tests (unit-mocked runner behavior plus real-DB integration suites gated behind `TEST_DATABASE_URL` that skip when unset). Runs with `npm run test` (230 passing; 22 database integration tests skip when `TEST_DATABASE_URL` is unset).
