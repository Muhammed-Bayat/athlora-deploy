...
- Public schedule: unauthenticated `GET /api/v1/public/schedule/clubs` and `GET /api/v1/public/schedule/clubs/:clubId` return only clubs with `publicScheduleEnabled = true` and their upcoming meet metadata (`date >= today`, `status IN ('scheduled','in_progress')` — title, date/time, venue, discipline, and selected disciplines only; never rosters, participants, or results). Each event carries `disciplines: [{ code, label }]` derived from its non-cancelled catalogue sessions (deduplicated by code), falling back to the legacy `discipline` scalar when no sessions exist. Unknown or unpublished clubs share the generic `404 NOT_FOUND`. The results flag alone never gates these routes, and the schedule flag alone never gates public statistics.
