...
- Tests: Vitest + Supertest (app routes, athlete and event route suites), middleware/application-user resolution, ownership/non-disclosure checks, athlete and event service/query parsing, validation and row-mapping, result derivation, and migration tests (unit-mocked runner behavior plus real-DB integration suites gated behind `TEST_DATABASE_URL` that skip when unset). Runs with `npm run test` (207 passing; 17 database integration tests skip when `TEST_DATABASE_URL` is unset).
