...
- **Public landing experience**: semantic React/HTML content remains separate from one lazy-loaded React Three Fiber canvas. The desktop hero reveals the approved SDP-Landing track exactly — a 2D SVG oval (8 gradient lanes, start line, distance marks, lane numbers and javelin throw sector) shown in a soft-feather circular window that follows the pointer using the mockup's static alpha mask (so there is no hard lens ring) alongside a matching cursor glow; the SVG is tilted with a strong perspective for a more 3D read. From the features section downward, the lazy-loaded Three.js scene drives a scroll-based lap in which a stylized humanoid sprinter is chased around a clearly-lane-marked, enlarged oval by a drone camera; the sprinter is built from overlapping primitives so every joint connects and is painted in the logo's crisp cyan/blue palette (vibrant cyan `#5CCCFE` torso and shins, deeper blue `#5AC9FE` forearms and thighs, navy `#001D3C` head and feet — no clothes, no skin). The sprinter starts exactly on the start line, and one full lap (back to the start line) completes precisely when the page is scrolled to the bottom; the testimonial and call-to-action bands above the footer are kept slightly translucent so the runner and track remain visible at the bottom. Runner progress and heading are eased (heading turns through the shortest arc, camera uses a capped frame-rate-independent lerp) so fast scrolling stays smooth and crisp, and the cyan accents are brightened (a lifted landing-cyan token and stronger 3D lane glow) while the dark aurora background is preserved. Mobile and reduced-motion modes use a static lower-page composition and hide the hero reveal. Actual scroll position enforces the hero/cinematic visibility boundary, and the scene does not change Auth0 or API behavior.
