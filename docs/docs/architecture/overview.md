...
- **Public landing experience**: semantic React/HTML content remains separate from one lazy-loaded React Three Fiber canvas. A shared procedural track model supports a masked desktop hero reveal and a scroll-driven drone camera from the features section downward; mobile and reduced-motion modes use a static lower-page composition. Actual scroll position enforces the hero/cinematic visibility boundary, and the scene does not change Auth0 or API behavior.
